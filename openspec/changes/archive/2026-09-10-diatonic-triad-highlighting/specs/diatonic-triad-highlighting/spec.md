...
## Purpose
TBD - created by change diatonic-triad-highlighting. Update Purpose
after archive.
...
"None" option (the default), replacing the previous boolean "Highlight
triad" toggle, showing each degree as a roman numeral whose case and
suffix reflect that degree's derived triad quality (upper-case for
major, lower-case for minor, lower-case with `°` for diminished,
upper-case with `+` for augmented).
...
in-scale note whose own degree label is in that set with the existing
triad outline ring — except the scale's root note, which keeps only
its existing root styling — composing with the existing dimmed and
active-press states exactly as it did for the previous root-only
highlighting.
