...
level — via position pills, a range minimap, zoom buttons, and touch
gestures anchored to the fretboard itself — independent of any
page-level or navigation state.

## ADDED Requirements

### Requirement: Position pills control the visible fret range
The system SHALL render position pills (`Pos 1` through `Pos 5`)
anchored along the top edge of the fretboard component. Selecting a
pill SHALL move and/or zoom the fretboard's visible fret window to that
position's fret span, on any viewport size.

#### Scenario: Selecting a position pill on desktop
- **WHEN** the user selects a position pill on a desktop viewport
- **THEN** the fretboard's visible fret window moves and/or zooms to
  that position's fret span

#### Scenario: Selecting a position pill on mobile
- **WHEN** the user selects a position pill on a viewport narrower than
  768px
- **THEN** the fretboard's visible fret window moves and/or zooms to
  that position's fret span, fitting it without requiring horizontal
  scrolling

#### Scenario: Manual adjustment after a pill selection is not overridden
- **WHEN** the user manually pans or zooms the fretboard after selecting
  a position pill, without selecting a different pill
- **THEN** the system does not re-apply the pill's fret span and
  override the user's manual adjustment
